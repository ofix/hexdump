...
TagWindow::TagWindow(wxWindow* parent,
                     wxWindowID id,
                     const wxPoint& pos,
                     const wxSize& size,
                     long style,
...
void TagWindow::OnClick(wxMouseEvent& event) {
    ...
    for (size_t iRow = iRowBegin; iRow < iRowEnd; iRow++) {
        for (size_t iCol = 0; iCol < m_visibleTags[iRow].size(); iCol++) {
            ...
            wxRect visible_rect(tag.rect.x, tag.rect.y - (iRowBegin) * (TAG_HEIGHT + TAG_MARGIN), tag.rect.width,
                                tag.rect.height);
            wxRect close_rect(visible_rect.x + TAG_PADDING + tag.name_width + 12, visible_rect.y + 8, 8,
                              tag.rect.height - 16);
            // 优先检查是否是关闭按钮
